    ...
    if request.method == 'POST':  
            ...
            if user is not None:  
                if user.is_active:  
                            ...
                            if user.username == '莫艳成':
                                return HttpResponseRedirect('/showrelease')
...
def svnupdate(request,svn_id,u_type):
    ...
    svnlog = os.path.join(settings.WEB_ROOT,'..\\','leopard','svnlog\\').replace('\\','/') +logname
    try:
...
@login_required(login_url='/login/')
def scriptdel(request,script_id):
...
def task_run(request):
    if request.method == 'POST':
        ...
        tasklogpath = os.path.join(settings.WEB_ROOT,'..\\','leopard','tasklog\\').replace('\\','/') +tasklogname
        try:
...
def task_status(request,task_id):
    ...
    tasklogpath = os.path.join(settings.WEB_ROOT,'..\\','leopard','tasklog\\').replace('\\','/') +task_log
    try:
...
def showservice(request):
    if request.method == 'POST':
        if request.POST['operation'] == 'log':
            ...
            host = service.host
            host_ip = host.host_w_ip
            host_pass = host.host_root_pwd
            host_port = host.host_w_port
            log = service.service_log
            cmd = 'tail -n 50 '+log
            print cmd
            result = ordinary_ssh(host=host_ip,username='root',password=host_pass,port=host_port,cmd=cmd)
            result = result.replace('\n','<br/>')
            print result
        elif request.POST['operation'] == 'del':
            service_id = request.POST['id']
            try:
                services.objects.filter(id=service_id).delete()
                result = 'delete service success!'
            except:
                result = 'delete service failed!'
        elif request.POST['operation'] == 'run':
            service_id = request.POST['id']
        ...
        return render(request, 'showservice.html',{'service_list':service_list})
...
def addservice(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            service_script = form.cleaned_data['service_script']
            service_log = form.cleaned_data['service_log']
            host = form.cleaned_data['host']
            try:
                services(service_name=service_name, service_script=service_script, service_log=service_log, host=host).save()
            except:
                result = "Add Service %s Failed!"%service_name
                form = serviceform()
                return render(request,'addservice.html',{'form':form,'result':result})
...
def addrelease(request):
    if request.method == 'POST':
        form = releasesform(request.POST,request.FILES)
        if form.is_valid():
            upload_file = request.FILES.get('upload_file')
            add_time = time.strftime('%Y-%m-%d %H:%M')
            add_reason = request.POST.get('add_reason')
            create_user = request.user
            releases(create_user=create_user,add_time=add_time,upload_file=upload_file,add_reason=add_reason).save()
            return HttpResponseRedirect('/showrelease/')
    else:
        form = releasesform()
